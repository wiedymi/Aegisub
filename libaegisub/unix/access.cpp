...
void Check(const std::string &file, acs::Type type) {
	...
	if (file_status != 0) {
		switch (errno) {
			case ENOENT:
				throw AcsNotFound(L"File or path not found.");
			break;

			case EACCES:
				throw AcsAccess(L"Access Denied to file, path or path component.");
			break;

			case EIO:
				throw AcsFatal(L"Fatal I/O error occurred.");
			break;
		}
	}
	...
	switch (type) {
		...
		case FileWrite:
			if ((file_stat.st_mode & S_IFREG) == 0)
				throw AcsNotAFile(L"Not a file.");
		break;

		case DirRead:
		case DirWrite:
			if ((file_stat.st_mode & S_IFDIR) == 0)
				throw AcsNotADirectory(L"Not a directory.");
		break;
	}
